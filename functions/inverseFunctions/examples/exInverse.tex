...
For all functions of the form \(f\left(x\right) = \frac{ax+b}{cx+d}\) we can find the inverse algebraically.
...
\begin{example}
    If we take the domain of \(f\left(x\right) = x^2\) to be \(\left[0,\infty\right),\) then \(f\) is one-to-one and the inverse is \(f^{-1}\left(x\right) = \sqrt{x}.\) Use this to find the inverse of \(f\left(x\right) = \frac{1}{x^2+4}\) on the domain \(\left[0,\infty\right).\)
    \begin{solution}
        \begin{gather*}
